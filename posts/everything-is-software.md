...
From a thermostat to a drainage model to a piece of music, anything that can be described with rules and data can be expressed as code. We've known this for decades, but what's changed is that *acting* on it no longer requires a computer science degree.

The boundaries between domains keep blurring. A software engineer builds a VR simulation of a physical space. A biologist writes code to model protein interactions. I analyze spatial data and build maps. A musician produces a track in a DAW that's really just a specialized IDE. We're all writing software, whether we call it that or not.
...
- A clinic building a lightweight app to triage patient intake, instead of bending a massive EHR system to fit
- A teacher spinning up a personalized quiz platform for their classroom
- A GIS analyst creating a focused field-data collection tool instead of wrestling with a one-size-fits-all enterprise suite (sorry Esri!)

These aren't side projects. They're purpose-built tools that solve real problems *because* the person closest to the problem is the one writing the code. No more waiting on a product roadmap that doesn't know your workflow exists.
